...
import { Plus, Minus, Package, TrendingUp, AlertTriangle, Settings } from 'lucide-react';
          ...
          <tbody className="bg-white/50 divide-y divide-slate-200/50">
            {rows.map((item) => (
              <tr key={item.id} className={`hover:bg-slate-50/80 transition-all duration-300 ${item.isLow ? 'bg-red-50/80' : ''}`}>
                ...
                <td className={`px-6 py-4 whitespace-nowrap text-sm text-slate-600 ${isRTL ? 'text-right' : 'text-left'}`}>
